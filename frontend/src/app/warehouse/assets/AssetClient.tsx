...
export default function AssetClient({ initialItems }: { initialItems?: Item.UiItem[] }) {
  ...
  useEffect(() => {
    ...
    const handleRefreshSignal = (message: string) => {
      if (message === 'ITEMS') refreshData();
    };
    socket.on("REFRESH_DATA", handleRefreshSignal);
    return () => { socket.off("REFRESH_DATA", handleRefreshSignal); };
  }, [refreshData]);
                  ...
                  <td className="px-6 py-4 w-[200px] text-right">
                    <div className="flex justify-end gap-1">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 text-white hover:bg-blue-600 rounded-lg transition-colors text-xs font-semibold"
                      >
