  ...
  const menus = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'รับสินค้าเข้าคลัง', path: '/warehouse/receives', icon: ArrowDownToLine },
      ]
    },
    {
              ...
              {group.items.map((menu) => {
                ...
                return (
                  <li key={menu.path} className="relative group">
                    <Link href={menu.path} className={`flex items-center ${collapsed ? 'justify-center' : 'justify-start gap-3'} px-3 py-2 rounded-lg transition-all duration-150 ${active ? 'bg-blue-50 text-blue-700 font-medium' : 'text-slate-800 font-normal hover:bg-slate-50 hover:text-slate-900'}`}>
