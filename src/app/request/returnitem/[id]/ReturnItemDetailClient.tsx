      ...
      <div className="w-full px-6 py-6 flex flex-col flex-1">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-violet-600 rounded-lg shrink-0">
                      ...
                      {attachmentThumbs.map((att, i) => {
                        ...
                        if (isImg) {
                          return (
                            <div key={`${att.url}-${i}`} className={wrapClass}>
                              <button type="button" aria-label={`ขยายรูป ${fname}`} onClick={() => setAttachmentLightbox({ url: att.url, name: fname })} className="w-full text-left rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-1">
                        ...
                        return (
                          <a key={`${att.url}-${i}`} href={att.url} target="_blank" rel="noopener noreferrer" className={wrapClass}>
                            <div className={thumbBoxClass}><FileText className="w-8 h-8 text-slate-300" /></div>
                            <p className="text-[11px] text-slate-600 mt-1.5 line-clamp-2 break-all text-center leading-tight" title={fname}>{fname}</p>
                          </a>
                        );
                      })}
