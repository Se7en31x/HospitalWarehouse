...
export default function BorrowClient({ initialItems }: Props) {
  ...
  const handleItemDetailConfirm = useCallback(
    (quantity: number) => {
      ...
      setCartItems((prev) => {
        ...
        if (exist) {
          ...
          return newQty > selectedItemForDetail.stock
            ? prev
            : prev.map((i) => (i.id === selectedItemForDetail.id ? { ...i, quantity: newQty } : i));
        }
        ...
        <div className="relative w-full sm:w-auto" data-filter-status>
          ...
          >
            <span className="text-slate-800 font-medium">
              {selectedStatus === "สถานะทั้งหมด" ? "สถานะทั้งหมด" : selectedStatus}
            </span>
