...
interface StockOutReportClientProps {
	onBack?: () => void;
}
...
const StockOutReportClient: React.FC<StockOutReportClientProps> = ({ onBack }) => {
	...
	const loadMovements = useCallback(async () => {
		setIsFetching(true);
		try {
			const data = await getAllStockMovements({ type: "OUT" });
			setMovements(data);
		} catch {
			SweetAlertUtils.error("เกิดข้อผิดพลาดในการโหลดข้อมูลจ่ายออก");
			setMovements([]);
		} finally {
			setIsFetching(false);
		}
	...
	const handleExportCsv = () => {
		const rows = [
			["รหัสสินค้า", "ชื่อสินค้า", "จำนวน", "ประเภทการจ่ายออก", "ผู้ดำเนินการ", "วันที่และเวลา", "หมายเหตุ"].join(","),
			...filteredMovements.map((movement) => [
				movement.item?.code || "-",
				movement.item?.name || "-",
				`${movement.quantity} ${movement.item?.unit || ""}`.trim(),
				movement.reason || movement.source_type || "-",
				movement.created_by || "-",
		...
		anchor.href = url;
		anchor.download = `รายงานจ่ายออก_${new Date().toISOString().slice(0, 10)}.csv`;
		anchor.click();
		...
		<div className="flex flex-col min-h-screen bg-white p-8">
			<div className="flex items-center justify-between mb-6">
				<div className="flex items-center gap-4">
					<h2 className="text-3xl font-bold text-gray-800">รายงานจ่ายออก</h2>
				</div>
			...
			<div className="flex flex-wrap gap-3 mb-6 items-center">
				<div className="relative w-72">
					...
					<input
						type="text"
						placeholder="ค้นหารหัสสินค้า / ชื่อสินค้า / ผู้ดำเนินการ..."
						value={searchTerm}
					...
					<table className="w-full text-sm text-left table-fixed">
						<thead>
							<tr className="bg-slate-50 text-slate-700 text-[13px] font-semibold uppercase border-b border-slate-300 sticky top-0 z-10">
								...
								<th className="px-6 py-4 w-[140px]">จำนวน</th>
								<th className="px-6 py-4 w-[160px]">ประเภทการจ่ายออก</th>
							...
							{paginatedMovements.length > 0 ? (
								paginatedMovements.map((movement, index) => (
									<tr key={movement.id} className="hover:bg-slate-50 transition-colors">
										...
										<td className="px-6 py-5 w-[160px] text-slate-700 font-normal">{movement.reason || movement.source_type || "-"}</td>
								...
								<tr>
									<td colSpan={7} className="text-center py-12">
										...
										<p className="text-sm text-slate-500">ไม่พบรายการจ่ายออก</p>
									</td>
								</tr>
							)}
						</tbody>
...
export default StockOutReportClient;